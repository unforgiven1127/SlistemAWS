  ...
  public function get_revenue_data($request_date = '', $location = '')
  {
    ...
    if ($read)
    {
      ...
      $query = "SELECT l.*, sln.shortname as nationality
                ...
                WHERE (l.position = 'Researcher' OR l.loginpk = '457') AND l.loginpk != '382'";
...
//var_dump($query);
    ...
    while($read)
    {
      ...
      if($temp['min_date'] == $temp['sl_meetingpk'] && $temp['min_date_position'] == $temp['sl_position_linkpk'] && $temp['meeting_done'] == 1 && $temp['pl_status'] == 51 && $temp['pl_active'] == 0 & $diff < 180) // && $diff < 180 geri koydum neden ciktiysa
      {
        if($group == 'researcher')
        {
          $user = $temp['created_by'];

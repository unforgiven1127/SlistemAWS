...
//ChromePhp::log($sLink);
            ...
            if($meetingDoneFlag)
            {
              $sMeeting.= $this->_oDisplay->getBloc('', ' updated'.$asDate[0].'</span> at <span>'.substr($asDate[1], 0, 5).'</span> ', array('style' => 'width:auto;','style' => 'margin-left:5px;' 'class' => 'meeting_row_sixth'));
            }

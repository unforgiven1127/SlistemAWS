    ...
    else*/
    {
      ...
      if(isset($pasHeader['lastNote']))
      {
        ...
        if(!empty($pasData['note_date']))
        {
          ...
          $asOption['title'] = "<div class=\'list_note_title\'>Last entry on the <span>".$pasData["note_date"]."</span><button onclick='$('.closePopUp').remove();' type='button' style='float: right;'>x</button></div>". $pasData["note_content"];

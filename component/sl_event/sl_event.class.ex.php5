    ...
    else
    {
      ...
      foreach($asNotes as $asNote)
      {
        ...
        $sHTML.= $oHTML->getBlocStart('', array('class' => 'entry'));
          ...
          {
            $sHTML.= $oHTML->getBloc('', '<span class="note_innerTitle">'.$asNote['title'].'</span><br /><textarea class="note_innerContent">'.
                    $asNote['content'].'</textarea>', array('class' => 'note_content test'));
          }
          else

...
//---->february in none leap years gave a remainder of 0 but still calculated weekends between first and last day, this is one way to fix it
  ...
  function getCharacterNotes($candidate_id)
  {
    ...
    $sQuery = "SELECT * FROM event e INNER JOIN event_link el on el.eventfk = e.eventpk WHERE el.cp_pk = '".$candidate_id."' AND e.type in('character') AND e.date_create > '".$sDate."'  AND LENGTH(e.content) >= 250";
